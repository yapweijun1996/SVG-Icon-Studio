...
3a. Re-activating the already-selected Category is a no-op: it must not collapse an expanded result batch, rebuild the catalogue, or repeat the unchanged polite result announcement; focus stays on the active chip.
...
4c. Advanced **Clear filters** keeps focus on its persistent control. When query/category/style context changes, it produces exactly one updated result-status announcement; when activation only resets Sort or is a no-op at defaults, it must not repeat an unchanged result announcement.
...
5g-a. Automatic IntersectionObserver pagination must preserve keyboard focus when its rerender replaces a focused Category chip or icon-card action; the equivalent control remains focused across each automatic batch while the auto-load itself stays silent in the result live region.
...
5i. Selecting an icon, opening More, or toggling Favorite outside the Favorites view must not mutate the catalogue result live region when result semantics are unchanged; unfavoriting/refavoriting inside Favorites still produces exactly one updated contextual result announcement.
5j. With only Category or Style filtering active and no search query, the results reset control is visible as **Clear filters** (not “Clear search”); activating it restores All category/all styles/default sort, hides the reset control, moves focus to the updated results heading, and then leaves the polite result status synchronized. When a search query is active, the same disappearing reset control instead returns focus to the persistent search field so the search workflow remains available.
5k. Rapid toast feedback remains capped at three visible messages; the toast live region stays `polite` but is non-atomic and additions-only, so each new toast is the relevant changed node and oldest-first eviction does not make prior/removal content eligible for re-announcement.
5l. At the mobile breakpoint, burst toast feedback begins below the sticky topbar (including the top safe-area inset), so three concurrent toasts never visually cover the menu/import/theme/inspector controls.
