...
Uploaded SVGs are a separate browser-local library. Metadata and sanitized SVG assets are stored in separate IndexedDB object stores. Existing legacy localStorage uploads are migrated once and removed only after successful migration. Canonical built-in icon IDs always take precedence if stale or legacy local data uses the same ID. Startup reconciliation removes metadata-only IndexedDB orphans whose SVG payload is already gone, while preserving asset-only SVG payloads to avoid deleting potentially recoverable user content. Orphan cleanup is best-effort: a cleanup write failure never hides otherwise valid uploaded icons. Direct v1 database upgrades remove each successfully migrated legacy row in the same transaction; incomplete legacy rows are retained for possible recovery.
...
- Search and generated-code controls expose explicit accessible names/states rather than relying on visual-only hints.
