...
5j. With only Category or Style filtering active and no search query, the results reset control is visible as **Clear filters** (not “Clear search”); activating it restores All category/all styles/default sort, hides the reset control, and leaves the result status synchronized.
